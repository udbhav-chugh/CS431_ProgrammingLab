...
public class keyHighlighter extends SwingWorker<Integer, Integer> {
  ...
  protected void process(final List<Integer> chunks) {
    for (Integer chunkNumber : chunks) {
        ...
        if(chunkNumber == 0){
          numOrOpButton[11].setBackground(colorGrey);
        }
        else{
        ...
        if(chunkNumber == 0){
          numOrOpButton[3].setBackground(colorGrey);
        }
        else{
          numOrOpButton[chunkNumber-1].setBackground(colorGrey);
        }
  ...
  protected Integer doInBackground() throws Exception {
    while(true){
      if(typeOfKey == 0) //for numbers,evaluate and clear
        i = (i+1)%12;
      else if(typeOfKey==1) //for operators
        i = (i+1)%4;
    publish(i);
    Thread.sleep(1200);
    }
